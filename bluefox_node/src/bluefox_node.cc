...
string _calib_file;
int _binning_v, _binning_h, _binning_type;
...
int process_inputs(const ros::NodeHandle &n)                                                                                                                           
{
  ...
  n.param("binning_v"    , _binning_v   , 0);
  n.param("binning_h"    , _binning_h   , 0);
  n.param("binning_type" , _binning_type, 0);
  ...
  _camInfo.binning_x = _binning_h;
  _camInfo.binning_y = _binning_v;
  ...
  ROS_INFO("Binning-[v, h, type] : [%d, %d, %d]" , _binning_v, _binning_h, _binning_type);
...
int main(int argc, char* argv[]){
  ...
  while (nh.ok()) {
    ...
    if(bfcam.grab_frame(frame, _print_stats) == -1){
      rate.sleep();
      continue;
    }

    // If parameter server reads <=0 values, image is publish without resizing
    if(_height > 0 && _width > 0){
      if(frame.rows != _height || frame.cols != _width)
        resize(frame, frame, Size(_width, _height));
    } else {
      _camInfo.roi.height = _camInfo.height = _height = frame.rows;
      _camInfo.roi.width  = _camInfo.width  = _width  = frame.cols;
    }
