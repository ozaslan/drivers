...
kQuadInterface::kQuadInterface()
{
  ...
  //gpsNanohexDataC.SetQueueLength(5);
                             ...
                             uint8_t * dst, int dstLen)
{
  ...
  //FIXME: hack cmd4 to send the camera trigger (empty packet with id 0, type 240)!
  uint32_t delay = 1;  // LED remains on for (delay) microseconds NOTE : this is blocking!
  int len2 = kBotPacket2WrapData(0, 240, (uint8_t*)&delay, sizeof(delay), dst, dstLen);
  return len2;
...
int kQuadInterface::ProcessIncomingPacket(kBotPacket2 * packet)
{
  ...
  if (id == KQUAD_LL_DEVICE_ID)
  {
    switch (type)
    {
      ...
      case 40 : pressMagDataC.PushData(data,len,40);    break;
      case 41 : quadStatusDataC.PushData(data,len,41);  break;
      case 42 : rcDataC.PushData(data,len,42);          break;
      case 170: gpsNanohexDataC.PushData(data,len,170); break;
    }
  }
  else if (id == 5)
  {
...
int kQuadInterface::SetMaxQueueLength(uint32_t maxLen)
{
  ...
  gpsNanohexDataC.SetQueueLength(maxLen);
...
int kQuadInterface::GetGpsNanohexData    ( list<GpsData> & dataList, uint32_t sizer)
{
  return  gpsNanohexDataC.GetData(dataList,sizer);
}
...
void kQuadInterface::PrintGpsData ( list<GpsData> & dataList)
{
  int size = dataList.size();
  if (size < 1) return;
  list<GpsData>::iterator it = dataList.begin();
  while (size--) (*(it++)).Print();
}
